...
SD_Error SD_Init(void) {
	...
	RCC_AHB1PeriphClockCmd(SDIO_DMA_CLK, ENABLE);//DMA2时钟

	RCC_APB2PeriphClockCmd(SDIO_RCC_CLK, ENABLE);//SDIO时钟使能
	...
	GPIO_Init(SDIO_D0_PORT, &GPIO_InitStructure);//复用功能输出

	GPIO_InitStructure.GPIO_Pin = SDIO_CMD_PIN;
	GPIO_Init(SDIO_CMD_PORT, &GPIO_InitStructure);//PD2复用功能输出

	//引脚复用映射设置
	GPIO_PinAFConfig(SDIO_CMD_PORT, SDIO_CMD_PSE, GPIO_AF_SDIO);
	GPIO_PinAFConfig(SDIO_CLK_PORT, SDIO_CLK_PSE, GPIO_AF_SDIO);
	GPIO_PinAFConfig(SDIO_D0_PORT, SDIO_D0_PSE, GPIO_AF_SDIO);
	GPIO_PinAFConfig(SDIO_D1_PORT, SDIO_D1_PSE, GPIO_AF_SDIO);
	GPIO_PinAFConfig(SDIO_D2_PORT, SDIO_D2_PSE, GPIO_AF_SDIO);
	GPIO_PinAFConfig(SDIO_D3_PORT, SDIO_D3_PSE, GPIO_AF_SDIO);

	RCC_APB2PeriphResetCmd(SDIO_RCC_CLK, DISABLE);//SDIO结束复位

	//SDIO外设寄存器设置为默认值
	SDIO_Register_Deinit();

	NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = SDIO_PreemptionPriority;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = SDIO_SubPriority;
...
void SD_DMA_Config(u32*mbuf, u32 bufsize, u32 dir) {
	...
	while (DMA_GetCmdStatus(SDIO_STREAM_X) != DISABLE) {} //等待DMA可配置

	DMA_DeInit(SDIO_STREAM_X);//清空之前该stream3上的所有中断标志


	DMA_InitStructure.DMA_Channel = SDIO_CHANNEL_NUM;  //通道选择
	...
	DMA_Init(SDIO_STREAM_X, &DMA_InitStructure);//初始化DMA Stream

	DMA_FlowControllerConfig(SDIO_STREAM_X, DMA_FlowCtrl_Peripheral); //外设流控制

	DMA_Cmd(SDIO_STREAM_X , ENABLE); //开启DMA传输
