...
void MP3GetLastFrameInfo(HMP3Decoder hMP3Decoder, MP3FrameInfo *mp3FrameInfo)
{
	...
	if (!mp3DecInfo || mp3DecInfo->layer != 3) {
		...
		mp3FrameInfo->number_out_samples =  (mp3DecInfo->nGrans) * (mp3DecInfo->nGranSamps) * (mp3DecInfo->nChans);  //nGrans * nGranSamps * nChans
	}
}
...
//		padbit = CheckPadBit(mp3DecInfo);
//		mp3DecInfo->nSlots = mp3DecInfo->freeBitrateSlots + padbit; //zkrt_debug
		mp3DecInfo->nSlots = mp3DecInfo->freeBitrateSlots + CheckPadBit(mp3DecInfo);	/* add pad byte, if required */
		if(mp3DecInfo->nSlots <0)  //zkrt_debug
			return ERR_MP3_LIB_ERROR;
	}
...
//			if(mp3DecInfo->nSlots <0) //zkrt_debug
//				mp3DecInfo->nSlots =0; //:这里发现Finease歌曲调用到这里时hardfault，数组溢出，应该是库存在问题。暂时未能解决此问题。
