...
//		LCD_ShowString(60+110+32,210,200,16,16,"Kbps");	 
		...
		printf("%d:%d/%d:%d%d Kbps\n", playtime/60, playtime%60,totsec/60, totsec%60,bitrate/1000);
	} 		 
}
//播放音乐
...
//		Show_Str(60,190,240-60,16,fn,16,0);				//显示歌曲名字 
		...
		if(key==KEY_STOP_SAVE)		//上一曲
		{
			if(curindex)curindex--;
			else curindex=totwavnum-1;
 		}else if(key==KEY_REC_PAUSE)//下一曲
		{
			curindex++;		   	
